...
Character-level language models built from scratch — from bigram counting to a WaveNet-inspired transformer. Trained on a dataset of names to generate new, realistic-sounding names.
...
jupyter notebook makemore.ipynb         # Part 1: Bigrams
jupyter notebook makemore_part2_mlp.ipynb  # Part 2: MLP
```

The `names.txt` dataset is included.

## Progression

| Part | Notebook | Model | Key Concept |
|------|----------|-------|-------------|
| 1 | `makemore.ipynb` | Bigram | Counting, softmax, negative log-likelihood loss |
| 2 | `makemore_part2_mlp.ipynb` | MLP | Embeddings, hidden layers, train/val/test splits |
| 3 | `makemore_part3_bn.ipynb` | MLP + BatchNorm | Kaiming init, activation diagnostics, batch normalization |
| 4 | `makemore_part4_backprop.ipynb` | MLP | Manual backpropagation — calculus from scratch |
| 5 | `makemore_part5_cnn1.ipynb` | WaveNet-style | Dilated convolutions, hierarchical input fusion |
...
* We end up with terrible predictions. This is because the bigram model is not good at predicting the next letter in a sequence of letters. It is good at predicting the next word in a sequence of words, but not the next letter in a sequence of letters.
...
* We then make the softmax function from scratch with differentiable operations so that we can do backpropagation on it.
...
* With Torch library, there are many ways to do the same thing. For example, we can use torch.nn.functional.one_hot to one-hot-encode the input, or we can use torch.nn.Embedding to do the same thing.
* To optimize the model we train it on many mini-batches of examples, which makes the training much faster, but the gradient direction is slightly less accurate. Even though this is still very useful.
...
* Since our dataset is so small we might be underfitting. To prevent this we can add more hidden layers, or increase the number of neurons in the hidden layer. 
  * By decreasing the learning rate, managed to improve the model a lot.
...
* Tanh just "squashes" the input values between -1 and 1.
  * Many of the values become -1 and 1, which is not good, this causes the gradient to be 0, which means the model cannot learn.
...
PyTorch significantly speeds up the process of training neural networks, but the torch.backward() has taken away much of what makes backward propagation so powerful.
<hr />
...
* Part 4's code follows from part 3.
  * To PyTorchify the code further, the forward pass is simplified and used containers.
...
* Instead of "squishing" all the inputs into one layer, the WaveNet architecture does it slowly, by fusing two inputs into one layer, then fusing the fused layer with another input, and so on.
  * This is done by using dilated convolutions.
    ...
    * WaveNet uses convolutional layers, we do not. It is simply for efficiency.
...
WaveNet architecture, is an autoregressive model used to predict audio; although, we built a hierarchical model that mimics it, but does not replicate it exactly, to improve the model's performance. 
